"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var common_1 = require('@angular/common');
var caterer_routing_1 = require('./caterer.routing');
var caterer_component_1 = require('./caterer.component');
var catererview_component_1 = require('./catererview.component');
var caterer_service_1 = require('./caterer.service');
var CatererModule = (function () {
    function CatererModule() {
    }
    CatererModule = __decorate([
        core_1.NgModule({
            imports: [common_1.CommonModule, caterer_routing_1.caterersRouting],
            declarations: [caterer_component_1.CatererComponent, catererview_component_1.CatererViewComponent],
            providers: [caterer_service_1.CatererService]
        }), 
        __metadata('design:paramtypes', [])
    ], CatererModule);
    return CatererModule;
}());
exports.CatererModule = CatererModule;
//# sourceMappingURL=caterer.module.js.map